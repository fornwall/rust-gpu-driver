...
pub fn split_input(
    ...
    let source_path_from_package = source_path
        .to_str()
...
*/
fn find_short_comment_manifest(s: &str) -> Option<Manifest> {
    ...
    */
    if let Some(cap) = re.captures(s) {
        if let Some(m) = cap.get(1) {
            return Some(Manifest::DepList(m.as_str()));
        }
    }
    None
}
...
*/
fn default_manifest(bin_name: &str, bin_source_path: &str) -> toml::value::Table {
    ...
    let mut lib_map = toml::map::Map::new();
    lib_map.insert(
        "crate-type".to_string(),
        toml::value::Value::Array(vec![toml::value::Value::String("dylib".to_string())]),
    );
    lib_map.insert(
        "path".to_string(),
    ...
    dependencies_map.insert(
        "spirv-std".to_string(),
        toml::value::Value::String("0.9".to_owned()),
    );
    ...
    mani_map.insert("lib".to_string(), toml::value::Value::Table(lib_map));
    mani_map.insert(
        "dependencies".to_string(),
        toml::value::Value::Table(dependencies_map),
    );
    //mani_map.insert( "profile".to_string(), toml::value::Value::Table(profile_map),);
